        ...
        <div class="border rounded-xl shadow-sm p-6 bg-white dark:bg-gray-800 dark:border-gray-700">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ consulteeRecordResponse.other }}</dd>
                </div>
...
export default {
    ...
    mounted() {
        if (!this.consulteeRecordResponse && this.consulteeRecordResponseId) {
            this.fetchConsulteeRecordResponse();
        } else {
            this.dataLoaded = true;
        }
    },
    methods: {
        fetchConsulteeRecordResponse() {
            showModel(this.consulteeRecordResponseId).then(res => {
