...
export const API_ROUTE_PREFIX = 'api.innoboxrr.consultant.manager.consultee_record_response.'; // Reemplaza con la ruta adecuada
	...
	return [
		{
			id: 'create',
			name: 'Create',
			callback: 'createConsulteeRecordResponse',
			icon: 'fa-plus',
			route: true,
			policy: false,
			params: {
...
export const showModel = (modelId, loadRelations = [], loadCounts = [], data = {}) => {
    return makeHttpRequest('get', route(API_ROUTE_PREFIX + 'show'), {
        _token: CSRF_TOKEN,
        consultee_record_response_id: modelId,
...
export const deleteModel = (data) => {
    ...
    return makeHttpRequest('post', route(API_ROUTE_PREFIX + 'delete'), {
        _token: CSRF_TOKEN,
        _method: 'DELETE',
        consultee_record_response_id: data.id,
