...
<script>
import { deleteModel } from '@consultantManagerModels/consultant';
export default {
    props: {
        consultant: {
            type: Object,
    ...
    created() {
        if (!this.consultant && !this.consultantId) {
            console.error("Se debe proporcionar 'consultant' o 'consultantId'.");
        }
    },
    methods: {
        deleteConsultant() {
            deleteModel(this.consultant).then(() => {
                this.$router.push({ name: 'AdminConsultants' });
            });
        }
    }
}
