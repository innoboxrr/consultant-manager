...
<script>
import { deleteModel } from '@consultantManagerModels/consultee';
export default {
    props: {
        consultee: {
            type: Object,
    ...
    created() {
        if (!this.consultee && !this.consulteeId) {
            console.error("Se debe proporcionar 'consultee' o 'consulteeId'.");
        }
    },
    methods: {
        deleteConsultee() {
            deleteModel(this.consultee).then(() => {
                this.$router.push({ name: 'AdminConsultees' });
            });
        }
    }
}
